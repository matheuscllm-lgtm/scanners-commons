...
- **Gotchas do PriceCharting (2026-09-12; valem também para COMC e outlook, que leem a mesma página):** (1) a **busca** (`search-products`) não resolve muitos slugs — use a URL direta da carta (`pc_url` da watchlist do eBay). (2) `VGPC.chart_data` (série mensal por grau, em centavos) é **preenchida para trás** com o primeiro valor em produto lançado depois de ago/2021 — ignore pontos anteriores ao lançamento. (3) A linha `sales_volume` usa `data-show-tab="completed-auctions-<grau>"` com hífen (`box-only`, `manual-only`): regex precisa de `[a-z-]+`. (4) Série PSA 10 de vintage tem ~1 venda/mês: ponto isolado pode ser outlier. (5) A regra do eBay PSA de **idioma explícito no título da venda** (nunca presumido) descarta a **maioria** das vendas PSA 10 da página — no piloto de 2026-09-12 o grupo WotC ficou com 0 candidatas por isso. É política de honestidade, não bug; só o operador muda.
