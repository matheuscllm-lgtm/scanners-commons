...
- **CardTrader:** (a) usar preço *per-expansion* cru em vez de *per-blueprint* dá **76% de falso positivo** → guarda = `validate_per_blueprint`. (b) Variante errada (reverse holo lida como holo) inflava (Gengar US$146 vs US$1599) → guarda `_rarity_is_holo`. (c) Trainer Gallery / Galarian Gallery (`TG##`/`GG##`) inflam 5–10× → filtro por regex. (d) **`reverseHolofoil` market é LIXO pra carta barata (common/uncommon vintage):** o listing É reverse de verdade (foil=True), então a escada de variante referencia `reverseHolofoil` corretamente — mas o "market" da pokemontcg.io pra reverse de common antiga é um número ralo/outlier, longe de qualquer liquidez. Scan vintage 2026-06-26: Lileep common normal $0.55 vs reverse "$37.50" (**68×**), Slugma $0.45→$25 (56×), Kakuna $0.65→$37 (57×); 16 dos 40 "deals" eram isso. A flag "Variante Baixa Confiança" do v2.18 **NÃO dispara** pra reverse genuíno (era fallback intencional só pra Holo Rare) → passavam como COMPRA limpa. **Guarda (v2.24, em PR):** se variante=`reverseHolofoil` E carta não-holo E `reverse.market > 5× normal.market` → flag baixa-confiança → "validar manual". Sinal de sanidade: olhar `low`/`normal` da variante; reverse 8–68× acima do normal = preço fantasma.
