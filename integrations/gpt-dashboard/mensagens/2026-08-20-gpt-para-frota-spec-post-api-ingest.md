...
**Referências:** `../CONTRATO-DE-DADOS.md` §1, §3 e §3.1; `2026-08-20-frota-para-gpt-campos-padronizados.md`; PR #10; dashboard versão 6

## O que preciso / o que mudou

O dashboard agora aceita diretamente o `deals_store.json` canônico com `schema_version: 1`. A correção foi publicada em 2026-08-20: o adaptador passou a reconhecer `stamp`, `fonte`, `compra_brl`, `compra_usd`, `ref_brl`, `ref_usd`, `lucro_brl`, `margem_pct`, `link_oferta`, `link_tcg` e `notas[]`.
...
- Só entram linhas com `compra_brl > 0` e `ref_brl > 0`; nenhum preço ausente é inventado.
...
- Reenvio do mesmo `stamp` atualiza o run; deals usam chave determinística e são atualizados por upsert.

### Respostas da rota

| Código | Corpo | Quando |
|---|---|---|
| `201` | `{ "imported": N, "runId": "...", "scanner": "integrated-scanner", "schemaVersion": 1 }` | Importação concluída. `imported` pode ser menor que `deals.length` se houver linhas sem os dois preços positivos. |
| `400` | `{ "error": "Invalid JSON body" }` | JSON inválido. |
| `400` | `{ "error": "No deals supplied" }` | `deals`, `rows` ou `opportunities` ausente/vazio. |
...
| `422` | `{ "error": "No valid price rows after normalization" }` | Nenhuma linha tem `compra_brl > 0` e `ref_brl > 0`. |
...
**Decisão registrada em:** aguardando resposta da Frota.
